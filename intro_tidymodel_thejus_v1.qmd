...
*Takeaway* - tidymodels brigns the tidyverse philosophy to machine learning!
...
-   `bake()` - Cook the data!
    ...
    **NOTE** - You can fit() a parnsnip specification directly as we've seen before, but when we use workflows (coming up!), fit() is called on the workflow instead giving it a logically strong standing.
    ...
    penguin_wf <- workflow() |> # workflow obj
      add_recipe(penguin_recipe) |> # Add recipe from above
...
-   The entire pipeline is one portable object
    ...
    **Wait... the accuracy is over the charts**. This is too good to be true. Let's investigate why.
    ...
    We predicted on the same exact data thate we trained on. Well, there happen to be a term for it **overfitting**.

    #### Usual Procedure

    To get a good assessment we first split our data provided by the **rsample** package. Only then we define our recipies, model, bundle, fit and evaulate.
